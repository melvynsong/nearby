...
/**
 * Service-role client for privileged server operations that must bypass RLS
 * (e.g. account lookup by phone, creating invited friend users).
 */
export function getServiceRoleSupabaseClient(): SupabaseClient {
  ...
  if (!url || !key) {
    throw new Error('SUPABASE_SERVICE_ROLE_KEY is missing for privileged server operation')
  }
